...
function FPCameraPlayerBase:stop_shooting( wait )
	self._recoil_kick.to_reduce 	= self._recoil_kick.accumulated * 1 --Feel free to change this to w/e multiplier (0-1)
	self._recoil_kick.h.to_reduce 	= self._recoil_kick.h.accumulated * 1  -- 1 = 100% recoil recovery, 0.5 = 50% recoil recovery, etc.
...
function FPCameraPlayerBase:_vertical_recoil_kick(t, dt)
	...
	if self._recoil_kick.current and self._recoil_kick.current ~= self._recoil_kick.accumulated then
		local n = math.step(self._recoil_kick.current, self._recoil_kick.accumulated, 100 * dt)
...
function FPCameraPlayerBase:_horizonatal_recoil_kick(t, dt)
	...
	if self._recoil_kick.h.current and self._recoil_kick.h.current ~= self._recoil_kick.h.accumulated then
		local n = math.step(self._recoil_kick.h.current, self._recoil_kick.h.accumulated, 100 * dt)
